...
public class Game {
	...
	Pair<String, Long> solve() {
		...
		for (int source = 0; source < sources.length; source++) {
			...
			if (crt.size() > 0) crt.get(0).vis();
		}
		StringBuilder commands = new StringBuilder();
		for (State s = best; s != null; s = s.prev) {
			commands.append(s.commands);
		}
		return Pair.make(commands.reverse().toString(), best.score);
	}
				...
				for (int x = 0; x < width; x++) {
					for (int rot = 0; rot < rotN; rot++) {
						if (can[y][x][rot] && lock[y][x][rot] >= 0) {
							...
							if (list.isEmpty() || list.get(0).expScore < score) {
