...
public class Game {
	...
	int W1 = 10, W2 = 100;
	double dW1 = 10, dW2 = 100;
	int maxSize;
	int count;
	
	void setW(double p) {
		double time = Utils.getTime() - sTime;
		int _W1 = W1, _W2 = W2;
		if (count++ < 5) return;
		if (time > p * TLE * 0.8) {
			dW1 *= 0.9;
			dW2 *= 0.9;
		} else if (time < p * TLE * 0.7) {
			dW1 *= 1.1;
			dW2 *= 1.1;
			if (dW1 > 1000) dW1 = 1000;
			if (dW2 > 1000) dW2 = 1000;
		}
		W1 = max(1, (int)round(dW1));
		W2 = max(1, (int)round(dW2));
		if (_W1 != W1 || _W2 != W2) {
			count = 0;
			System.err.println();
			System.err.printf("W1 = %d, W2 = %d%n", W1, W2);
		}
	}
...
//		double size = width * height * sources.length * avgRot;
		double w = 1e4 * TLE / size;
		W1 = min(1000, max(1, (int)round(w)));
		W2 = min(1000, max(1, (int)round(w * 10)));
		dW1 = W1;
		dW2 = W2;
		...
		System.err.printf("size = %.2e, W1 = %d, W2 = %d%n", size, W1, W2);
		double time = Utils.getTime() - sTime;
		sTime -= time;
		TLE -= time;
		if (TLE < 0.1) TLE = 0.1;
...
//		}
	...
	Pair<String, Long> solve1() {
		...
		State best = initialState;
		for (int source = 0; source < sources.length && crt.size() > 0; source++) {
			if (Utils.getTime() - sTime > TLE) break;
			System.err.printf("\r%03d/%03d %d %.1f %s", source, sources.length, crt.get(0).finalScore, crt.get(0).expScore, crt.get(0).commands);
			if (source > 10) {
				setW((double)source / sources.length);
			}
			ArrayList<State> next = new ArrayList<>();
			crt.get(0).vis();
			for (State s : crt) {
