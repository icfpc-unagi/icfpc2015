...
class ICFPC2015
{
    string version = "chokudAIver4.0.4";
    int DIV = 50;
    ...
    void beamSearch()
    {
        ...
        int starttime = (int)sw1.ElapsedMilliseconds;
        ...
        for (int t = 0; t < d.sourceLength; t++)
        {
            ...
            if (t > 10)
            {
                double targettime = starttime + (double)((reallimit - 50) - starttime) * t / d.sourceLength;
                double diff = sw1.ElapsedMilliseconds - targettime;
                //Console.Error.WriteLine(targettime + " " + sw1.ElapsedMilliseconds + " " + diff);
                if (diff > 0)
                {
                    minwidth = (int)Math.Max(1, Math.Max(minwidth * 9 / 10, Math.Min(minwidth, firstwidth / Math.Pow(4, diff * d.sourceLength / (d.sourceLength - t)))));
                    //Console.Error.WriteLine(minwidth);
                }
                beamwidth = minwidth;
            }
            ...
            foreach (var nowstate in nowsl)
            {
                ...
                while (q.top != null)
                {
                    ...
                    for (int pp = 0; pp < phrase.Count; pp++)
                    {
                        ...
                        if (biast.Length > 20 && beamwidth < 20) continue;
                        ...
                        {
                            bool skipflag = true;
                            if (stepnumber == 0 && beamwidth == 1 && ss > 0) ss = 0;
