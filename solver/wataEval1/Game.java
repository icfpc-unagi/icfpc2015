...
public class Game {
	...
	Pair<String, Long> solve() {
		...
		for (int source = 0; source < sources.length && crt.size() > 0; source++) {
			System.err.printf("\r%03d/%03d %d %.1f", source, sources.length, crt.get(0).finalScore, crt.get(0).expScore);
			...
			for (int y = 0; y < height; y++) {
				...
				for (int x = 0; x < width; x++) {
					...
					if (bs[y][x2]) {
						...
						if (maxSize > 1 && (used >> 0 & 1) != 0 && (used >> 3 & 1) != 0) dead *= 0.8;
						if (maxSize > 1 && (used >> 1 & 1) != 0 && (used >> 2 & 1) != 0) dead *= 0.8;
...
//						for (int a = 0; a < aho.N; a++) if (dp2[a] >= 0 && (p < 0 || dp2[p] < dp2[a] || dp2[p] == dp2[a] && aho.subs[p].length() < aho.subs[a].length())) {
		...
		void vis() {
			if (!DEBUG) return;
			for (int i = 0; i < width; i++) System.err.print("--");
			System.err.println("---");
